      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-16">
        ...
        <div className="flex flex-col gap-40 mb-32">
          {featuredProjects.map((project, index) => (
            ...
            >
              {/* Image Container */}
              <div className="w-full lg:w-3/5 aspect-[16/10] rounded-[40px] overflow-hidden relative group glass-panel">
                <div className="project-image absolute inset-0 flex items-center justify-center">
                  <span className="text-9xl font-bold text-jet-black/[0.05]">
                    {index + 1}
                  </span>
                </div>
                <div className="absolute inset-0 bg-burnt-peach/90 opacity-0 group-hover:opacity-100 transition-all duration-500 z-20 flex items-center justify-center">
                  <Link
              ...
              {/* Text Content */}
              <div className="project-card w-full lg:w-2/5 space-y-6 glass-panel rounded-[32px] p-8 md:p-10">
                <h3 className="text-4xl md:text-6xl font-extrabold text-jet-black font-jakarta tracking-tighter leading-none">
                  {project.title}
                </h3>
                <p className="text-lg text-jet-black/60 font-medium font-jakarta leading-relaxed">
                  {project.description}
                </p>
                <div className="text-xs font-bold uppercase tracking-widest text-burnt-peach/60">
                  {project.role}
                </div>
              </div>
            </div>
        ...
        <div className="flex justify-center mt-20">
          <Link
            href="/projects"
            className="group relative inline-flex items-center gap-6 px-14 py-7 bg-jet-black text-white rounded-full overflow-hidden transition-all duration-500 hover:scale-105"
          >
            {/* Effet de remplissage au hover (Background liquide) */}
            <div className="absolute inset-0 bg-burnt-peach translate-y-[101%] group-hover:translate-y-0 transition-transform duration-500 ease-[0.22,1,0.36,1]" />
