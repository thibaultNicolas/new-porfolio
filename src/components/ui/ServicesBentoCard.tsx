...
export function ServicesBentoCard({
  ...
  return (
    <article
      ...
      className={cn(
        "services-bento-card group relative flex min-h-[18.5rem] flex-col justify-between overflow-hidden rounded-[1.75rem] p-7 transition-[transform,box-shadow,border-color] duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] motion-reduce:transition-none md:min-h-[21rem] md:p-8",
        featured
    ...
    >
      ...
      <div ref={mediaRef} className="relative will-change-transform">
        ...
        <p
          className={cn(
            ...
            featured ? "text-paper/65 group-hover:text-paper/78" : "text-fg/55 group-hover:text-fg/72",
          )}
        >
          {description}
        </p>
      ...
      <span
        ...
        aria-hidden="true"
      >
        {featured ? <ArrowUpRightIcon /> : <ArrowRightIcon />}
      </span>
    </article>
  );
}
